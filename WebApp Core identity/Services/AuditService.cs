    ...
    public class AuditService
    {
            ...
            string? errorMessage = null)
        {
            try
            {
                ...
                _logger.LogInformation("Audit log created: {Action} from {IP}",
                    HttpUtility.HtmlEncode(action), HttpUtility.HtmlEncode(ipAddress));
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Failed to create audit log for action: {Action}", HttpUtility.HtmlEncode(action));
            }
        }
